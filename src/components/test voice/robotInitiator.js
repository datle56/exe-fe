...
export function useRobotInitiator(info, verbose, playerRef) {
  ...
  const onStartStage = React.useCallback(async () => {
    ...
    try {
      ...
      const { remaining_ai_conversations } = remainData;

      if (remaining_ai_conversations === 0) {
        setErrorMessage('You have no remaining AI conversations. Please purchase more sessions.');
        return;
      }
