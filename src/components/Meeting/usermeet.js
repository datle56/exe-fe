...
function Meet() {
  ...
  const checkRemainingSessions = async () => {
    const token = localStorage.getItem('token');
    const response = await fetch('https://speak.id.vn/api/get_remain', {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    if (!response.ok) {
      throw new Error('Failed to fetch remaining sessions');
    }
    return response.json();
  };

  const startConnection = async (selectedTopic) => {
    setErrorMessage('');
    ...
    try {
      const remainData = await checkRemainingSessions();
      const { remaining_learning_sessions } = remainData;

      if (remaining_learning_sessions === 0) {
        setShowCards(true);
        setErrorMessage('You have no remaining learning sessions. Please purchase more sessions.');
        return;
      }
      ...
      const wsUrl = `${protocol}//speak.id.vn/api/ws/${role}/${username}/${selectedTopic}`;
      ...
      newWs.onmessage = (event) => {
        ...
        if (response.redirect_url) {
          ...
          setLoading(false); // End loading effect when redirecting
        }
        if (response.message) {
      ...
      setShowCards(true);
      setErrorMessage('An error occurred while checking remaining sessions. Please try again later.');
    }
  };

  useEffect(() => {
    if (topic) {
      startConnection(topic);
    }
  }, [topic]);
                ...
                <div>
                        ...
                        >
                          <Card.Body style={{
                            ...
                            <Row style={{ marginTop: "80%", display: "flex", justifyContent: "center", opacity: "0.8" }}>
                              {!loading && <Button style={{ backgroundColor: "white", color: "black", fontWeight: "bold" }} onClick={() => { setTopic(item.text); }}>{item.text}</Button>}
                            </Row>
                          </Card.Body>
                        </Card>
                      </Col>
                  ...
                  {errorMessage && (
                    <Row style={{ marginTop: '2rem', justifyContent: 'center' }}>
                      <Col md={8} style={{ textAlign: 'center', color: 'red', fontWeight: 'bold' }}>
                        {errorMessage}
                      </Col>
                    </Row>
                  )}
